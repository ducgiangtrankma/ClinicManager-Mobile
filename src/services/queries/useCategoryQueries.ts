...
export const useCategoryListQuery = (limit: number, keyword?: string) => {
  const storeId = store.getState().facilityReducer.facility?.id ?? '';

  return useInfiniteQuery({
    queryKey: queryKeys.categoryList.listCategory(storeId, limit, keyword),
    ...
    select: data => {
      const flatCustomers = data.pages.flatMap(p => p.categories);
